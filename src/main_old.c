...
int main(int argc, char *argv[]) {
    ...
    if (mosq) {
        ...
        //     goto terminate;
        // }
        // Print the MQTT connection configuration
        flog(logfile, "Attempting to connect to MQTT broker at %s:%d\n", config.host, config.port);

        // Try to connect to the MQTT broker
        rc = mosquitto_connect(mosq, config.host, config.port, 60);

        // If the connection fails, print the error code and message
        if (rc) {
            flog(logfile,
                "Unable to connect to broker at %s:%d, error code: %d, error message: %s\n",
                config.host, config.port, rc, mosquitto_strerror(rc));
            goto terminate;
        } else {
            flog(logfile, "Successfully connected to MQTT broker at %s:%d\n", config.host, config.port);
        }
